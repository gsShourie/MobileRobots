...
public:
    ...
    //creating initwheel function in Robot class
    void initWheel(double, double, double);
    ...
    double wheelGap=0;
    bool wheelInitialized=0;
    double wheelAngVelLimit=10;
...
private:
    //move robot with a given (x,y,theta) velocity. Moved from public to private,
    void moveXYT(double _vx, double _vy, double _omega);  //so it can now only be accessed by members of this class
